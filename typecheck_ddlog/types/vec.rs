...
{   let ref mut res: crate::ddlog_std::Vec<B> = crate::ddlog_std::vec_with_capacity((&crate::ddlog_std::len_ddlog_std_Vec__X___Bitval64(v)));
    for x in v.iter() {
...
{   let ref mut del: crate::ddlog_std::s64 = (0 as i64);
    ...
    for i in crate::ddlog_std::range_vec((&(0 as i64)), (&((*len).clone() as i64)), (&(1 as i64))).iter() {
        {
            let ref mut x: A = crate::ddlog_std::unwrap_or_default_ddlog_std_Option__A_A((&crate::ddlog_std::nth_ddlog_std_Vec__X___Bitval64_ddlog_std_Option__X(v, (&((*i).clone() as u64)))));
            if (!f.call(x)) {
                (*del) = ((*del).clone().wrapping_add((1 as i64)))
            } else {
                if ((&*del) > (&*(&(0 as i64)))) {
                    {
                        crate::ddlog_std::update_nth(v, (&(((*i).clone().wrapping_sub((*del).clone())) as u64)), x);
                        ()
                    }
                } else {
                    ()
    ...
    if ((&*del) > (&*(&(0 as i64)))) {
        {
            crate::ddlog_std::truncate(v, (&((*len).clone().wrapping_sub(((*del).clone() as u64)))));
            ()
        }
    } else {
        ()
